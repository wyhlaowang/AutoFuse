...
def load(device='cuda:0'):
    ...
    dec = Decoder(1, 32*(2**4), 4, False, 16).to(device=device).eval()
...
def test(dev): 
    ...
    file_list = os.listdir(os.path.join(data_folder, ir_folder))
    ...
    with torch.no_grad():
        for i in file_list:
            ir = read_image(os.path.join(data_folder, ir_folder, i), ImageReadMode.GRAY).to(device=dev) / 255.
            vi = read_image(os.path.join(data_folder, vis_folder, i), ImageReadMode.RGB).to(device=dev) / 255.
